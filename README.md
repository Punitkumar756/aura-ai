...
- **Small, focused MVP:** Pipeline Guardian Agent + 150 lines of core logic
- **Real automation:** Creates MR, updates status, diagnoses in seconds
- **Extensible:** Router + tool pattern makes adding agents trivial
- **Production-ready:** Logging, error handling, signature validation
- **Measurable impact:** Reduces manual debugging time significantly

## Quick Wins for Your Hackathon

1. **Pipeline Guardian (done):** Auto-fix failed tests/lint
2. **Status Dashboard:** Show agent stats (runs, fixes, accuracy)
3. **Approval Gate:** Require human approval before merge
4. **Extended Diagnostics:** Integrate with real LLM for better analysis
5. **Compliance Agent:** Check for secrets exposure on MR
6. **Cost Savings Calc:** Show how many CI reruns prevented
